...
pub fn create_user(db: &DbPool, email: &str, api_key: &str, api_key_hash: &str) -> Result<User, String> {
    ...
    conn.execute(
        "INSERT INTO users (id, email, api_key, api_key_hash) VALUES (?1, ?2, ?3, ?4)",
        params![id, email, api_key, api_key_hash],
...
pub fn find_user_by_api_key_hash(db: &DbPool, hash: &str) -> Option<User> {
    let conn = db.lock().ok()?;
    conn.query_row(
        "SELECT id, email, api_key, plan, stripe_customer_id, stripe_subscription_id, created_at FROM users WHERE api_key_hash = ?1",
        params![hash],
        |row| Ok(User {
            id: row.get(0)?,
            email: row.get(1)?,
...
pub fn find_user_by_email(db: &DbPool, email: &str) -> Option<User> {
    let conn = db.lock().ok()?;
    conn.query_row(
        "SELECT id, email, api_key, plan, stripe_customer_id, stripe_subscription_id, created_at FROM users WHERE email = ?1",
        params![email],
        |row| Ok(User {
            id: row.get(0)?,
            email: row.get(1)?,
