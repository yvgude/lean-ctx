...
| **P4 — production SDK + Engine extraction** | **P4 ACTIVE / V4 dual-track:** private SDK staging candidate `62400bfb74` extends the technically reviewed `2bafc965d3` five-primitive/package/exception baseline with R2 observable-parity evidence and its pinned baseline correction; final R2 review remains open, and no remote or release exists. BSL 1.1 is the decided license family; exact parameters remain pending; provenance is `CLEAN_REIMPLEMENT`. Engine integration candidate `eb8db0ca23`, based on `github/main@49a6b2053c`, serializes reviewed R4A–R4E seams plus reviewed D1 orphan deletion; extraction closure remains open. The Apache Python Product lifecycle remains `OSS_TRANSITION_COMPAT`. | Close the SDK R2 review, run one Engine integration gate after the remaining bounded slices, and close ownership/extraction plus legal/release gates; do not begin P5/Cloud. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `ContextSession`, `ContextSource`, `ContextView`, `ContextPlan`, `ContextReceipt` | P3 Python Preview plus private SDK staging | Production SDK, BSL 1.1 family | `62400bfb74` (R2 final review pending; reviewed baseline `2bafc965d3`) | Canonical Product lifecycle not closed in Engine; historical `fbfec0b` evidence quarantined | Preserve the integrated Preview path; add no new Apache Product semantics | R2 review, contract/compatibility fixtures, publication/legal approval and rollback path |
