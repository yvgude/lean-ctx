...
| **P4 — production SDK + Engine extraction** | **P4 ACTIVE / final V4 candidate:** private SDK ledger `a7afd1f` records immutable implementation `11f77debc2` and wheel `b54ac013…`; Python 3.9/3.14 pass 19/19, the certified offline closure passes 25/25, and real Engine plus provider-free OpenAI Agents gates pass. BSL 1.1 parameters/publication remain explicit and pending; provenance is `CLEAN_REIMPLEMENT`. Local Engine candidate `e495b61f24` on `github/main@9aba08d407` completes R4A–R6, including factual R4F, frozen R4G, two R5 decommission batches and source-compatible R6 cleanup. The Apache Python lifecycle remains `OSS_TRANSITION_COMPAT`. | Run one final integrated gate, deliver the Engine stack through serial PR CI, record final main SHA, then stop; do not begin P5/Cloud. |
