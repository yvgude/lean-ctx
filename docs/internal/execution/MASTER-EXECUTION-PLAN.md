...
| **P4 — production SDK + Engine extraction** | **P4 ACTIVE / final V4 candidate:** private SDK `dbfe753` passed independent internal-RC review; immutable implementation `176c2aa2ab` and wheel digest are recorded, with Python 3.9/3.14, real Engine and provider-free OpenAI Agents evidence. BSL 1.1 parameters/publication remain explicit and pending; provenance is `CLEAN_REIMPLEMENT`. Local Engine candidate `e495b61f24` on `github/main@9aba08d407` completes R4A–R6, including factual R4F, frozen R4G, two R5 decommission batches and source-compatible R6 cleanup. The Apache Python lifecycle remains `OSS_TRANSITION_COMPAT`. | Run one final integrated gate, deliver the Engine stack through serial PR CI, record final main SHA, then stop; do not begin P5/Cloud. |
...
| R5 decommission | Detached stores and heuristic Product outcome research | Removed or frozen DTO compatibility | Explicit SDK/evaluator outcomes | 703 lines removed in two reviewed batches; no duplicate canonical lifecycle remains | Factual Engine measurements retained | One-commit rollback; next-major gate for retained DTOs |
