    ...
    for label in &["Function", "Method"] {
        ...
        for node in nodes {
            if let Some(fp_hex) = node.properties.get("fp") {
                if let Some(mh) = Minhash::from_hex(fp_hex) {
...
mod tests {
    ...
    fn setup_gbuf() -> GraphBuffer {
        ...
        gb.upsert_node("Function", "func_a", "pkg.func_a", "src/a.rs", 1, 10,
            props(&[("fp", &mh_hex_all(42))]));
        gb.upsert_node("Function", "func_b", "pkg.func_b", "src/b.rs", 1, 10,
            props(&[("fp", &mh_hex_all(42))]));
        gb
    }
    ...
    fn test_collect_fp_entries_filters_nodes() {
        let mut gb = GraphBuffer::new("test");
        // Function with fp
        gb.upsert_node("Function", "a", "pkg.a", "a.rs", 1, 5,
            props(&[("fp", &mh_hex_all(1))]));
        // Method with fp
        gb.upsert_node("Method", "b", "pkg.b", "b.rs", 1, 5,
            props(&[("fp", &mh_hex_all(2))]));
        // Function without fp
        gb.upsert_node("Function", "c", "pkg.c", "c.rs", 1, 5, props(&[]));
        // Class node (should be ignored)
        gb.upsert_node("Class", "d", "pkg.d", "d.rs", 1, 5, props(&[("fp", &mh_hex_all(3))]));
    ...
    fn test_collect_fp_entries_sorts_by_node_id() {
        let mut gb = GraphBuffer::new("test");
        let id_a = gb.upsert_node("Function", "a", "pkg.a", "a.rs", 1, 5,
            props(&[("fp", &mh_hex_all(1))]));
        let id_b = gb.upsert_node("Function", "b", "pkg.b", "b.rs", 1, 5,
            props(&[("fp", &mh_hex_all(2))]));
        let entries = collect_fp_entries(&gb);
    ...
    fn test_similar_to_no_edge_for_low_threshold() {
        ...
        gb.upsert_node("Function", "a", "pkg.a", "a.rs", 1, 5,
            props(&[("fp", &mh_hex_all(1))]));
        gb.upsert_node("Function", "b", "pkg.b", "b.rs", 1, 5,
    ...
    fn test_similar_to_only_function_and_method() {
        ...
        gb.upsert_node("Class", "c", "pkg.c", "c.rs", 1, 5,
            props(&[("fp", &mh_hex_all(42))]));
    ...
    fn test_max_edges_per_node() {
        ...
        let mut gb = GraphBuffer::new("test");
        let src_id = gb.upsert_node("Function", "src", "pkg.src", "src.rs", 1, 5,
            props(&[("fp", &mh_hex_all(42))]));
        for i in 0..20usize {
            gb.upsert_node("Function", &format!("tgt_{i}"), &format!("pkg.tgt_{i}"),
                "tgt.rs", 1, 5, props(&[("fp", &mh_hex_all(42))]));
        }
    ...
    fn test_similar_to_multiple_matches() {
        ...
        gb.upsert_node("Function", "b", "pkg.b", "b.rs", 1, 5,
            props(&[("fp", &mh_hex_all(42))]));
        gb.upsert_node("Function", "c", "pkg.c", "c.rs", 1, 5,
            props(&[("fp", &mh_hex_all(42))]));
