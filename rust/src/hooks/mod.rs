...
pub fn refresh_installed_hooks() {
    ...
    if claude_hooks {
        agents::install_claude_hook_scripts(&home);
        agents::install_claude_hook_config(&home);
    }
    ...
    if cursor_hooks {
        agents::install_cursor_hook_scripts(&home);
        agents::install_cursor_hook_config(&home);
    }
    ...
    if gemini_rewrite.exists() || gemini_legacy.exists() {
        agents::install_gemini_hook_scripts(&home);
        agents::install_gemini_hook_config(&home);
    }

    if home.join(".codex/hooks/lean-ctx-rewrite-codex.sh").exists() {
        agents::install_codex_hook_scripts(&home);
    }
}
...
pub(crate) const REDIRECT_SCRIPT_CLAUDE: &str = r#"#!/usr/bin/env bash
...
pub(crate) const REDIRECT_SCRIPT_GENERIC: &str = r#"#!/usr/bin/env bash
...
pub fn install_agent_hook(agent: &str, global: bool) {
    match agent {
        "claude" | "claude-code" => agents::install_claude_hook(global),
        "cursor" => agents::install_cursor_hook(global),
        "gemini" | "antigravity" => agents::install_gemini_hook(),
        "codex" => agents::install_codex_hook(),
        "windsurf" => agents::install_windsurf_rules(global),
        "cline" | "roo" => agents::install_cline_rules(global),
        "copilot" => agents::install_copilot_hook(global),
        "pi" => agents::install_pi_hook(global),
        "qwen" => agents::install_mcp_json_agent(
            "Qwen Code",
            "~/.qwen/mcp.json",
            &dirs::home_dir().unwrap_or_default().join(".qwen/mcp.json"),
        ),
        "trae" => agents::install_mcp_json_agent(
            "Trae",
            "~/.trae/mcp.json",
            &dirs::home_dir().unwrap_or_default().join(".trae/mcp.json"),
        ),
        "amazonq" => agents::install_mcp_json_agent(
            "Amazon Q Developer",
        ...
        "jetbrains" => agents::install_mcp_json_agent(
            "JetBrains IDEs",
            "~/.jb-mcp.json",
            &dirs::home_dir().unwrap_or_default().join(".jb-mcp.json"),
        ),
        "kiro" => agents::install_kiro_hook(),
        "verdent" => agents::install_mcp_json_agent(
            "Verdent",
            "~/.verdent/mcp.json",
            &dirs::home_dir()
                .unwrap_or_default()
        ...
        "opencode" => agents::install_mcp_json_agent(
            "OpenCode",
            "~/.opencode/mcp.json",
            &dirs::home_dir()
                .unwrap_or_default()
