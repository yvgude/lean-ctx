        ...
        tokio::task::block_in_place(|| {
...

            {
                let Some(mut session) = crate::server::bounded_lock::write_for(
                    ...
                    let extra_env = match collect_extra_env(args) {
                        Ok(env) => env,
                        Err(message) => {
                            return Ok(ToolOutput {
                                shell_outcome: Some(ShellOutcome::Blocked),
...
/// Collect the caller's `env` overrides, refusing protected keys by name instead
/// of dropping them silently (#1771).
///
/// The inline-override refusal in `core/shell_allowlist/enforcement.rs` points the
/// caller at this parameter and uses `PATH` as its worked example — yet five of the
/// nine variables it blocks inline (`PATH`, `GIT_SSH`, `GIT_SSH_COMMAND`,
/// `LD_PRELOAD`, `DYLD_INSERT_LIBRARIES`) are also in [`is_dangerous_env_key`].
/// Filtering them away meant the recommended recovery ran the command with the
/// ambient value and reported success, which is worse than the refusal it replaced:
/// a wrong result that looks right. Naming the key is the only honest outcome.
fn collect_extra_env(
    ...
    for (key, value) in obj {
        ...
        accepted.insert(key.clone(), text.to_string());
    }
    ...
    let names = rejected
        ...
        .collect::<Vec<_>>()
        .join(", ");
    let plural = if rejected.len() == 1 { "key" } else { "keys" };
    Err(format!(
        "ERROR: ctx_shell refuses the `env` {plural} {names} — nothing was run. These keys can \
         redirect which binary executes, so lean-ctx never forwards them, and the command would \
         otherwise have used the inherited value while appearing to honor yours. This is the \
         same rule that blocks the inline `KEY=value cmd` form; `env` is not a way around it. \
         Invoke the binary by its full path instead, or drop the override."
    ))
}
